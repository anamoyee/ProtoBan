...
_cleanup_on_shutdown: list[TimeoutDisableItemsViewMixin] = []

logger = get_logger(__name__)


class TimeoutDisableItemsViewMixin(BoundOnlyViewMixin):
	...
	def _client_start_hook(self, client: miru.Client) -> None:
		...
		if self.message.flags & hikari.MessageFlag.EPHEMERAL and self._response is None:
			msg_1 = "TimeoutDisableButtonsViewMixin cannot be started bound to an ephemeral message, unless you call provide_response_for_ephemeral_on_timeout(...) before starting the view."
